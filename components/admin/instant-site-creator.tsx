...
import { X, Sparkles, ArrowRight, Loader2, Store, Check, ExternalLink, Briefcase, Palette, User } from "lucide-react"
...
const SITE_TYPE_CONFIG = {
  ...
  other: { label: "Website", icon: Store, color: "text-gray-500" },
}

export function InstantSiteCreator({ isOpen, onClose, isFullscreen = false }: InstantSiteCreatorProps) {
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            exit={{ y: "100%" }}
            transition={transition}
            className={`absolute inset-x-0 bottom-0 bg-card z-50 border-t border-border flex flex-col overflow-hidden ${isFullscreen ? "top-0" : "top-24 rounded-t-xl"}`}
          >
            {/* Header */}
